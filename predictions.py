...
if __name__ == "__main__":
    ...
    while True:
        ...
        if input_action == 'exit':
            ...
            my_players = input("Enter player names separated by commas: ").split(',')
            my_players = [name.strip() for name in my_players]
            game_date = input("Enter game date (YYYY-MM-DD): ")
            lineup_size = input("Enter lineup size (default 5): ").strip()
            lineup_size = int(lineup_size) if lineup_size else 5
            
            # Fixed: only unpack 2 values (lineup, total_points)
            lineup, total_points = get_best_lineup(
                loaded_model, df, my_players, game_date, 
                lineup_size=lineup_size
            )
            
            print("\nBest Lineup:")
            for player_name, predicted_points in lineup:
                print(f"Player: {player_name}, Predicted Points: {predicted_points:.2f}")
